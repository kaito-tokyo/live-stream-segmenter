...
public:
     ...
     */
	void fetchStreamKeys(const std::string &accessToken, FetchKeysSuccess onSuccess, ErrorCallback onError);

private:
	// 内部実装: HTTPリクエストを実行する汎用メソッド
	void performGetRequest(const std::string &url, const std::string &accessToken,
			       std::function<void(const std::string &responseBody)> onSuccess, ErrorCallback onError);
};
